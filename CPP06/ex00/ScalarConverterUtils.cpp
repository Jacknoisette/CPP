...
/*   Updated: 2025/04/10 15:19:10 by jdhallen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	display_convert(float floating, int decimal){
	std::string info[2] = {"OK", "OK"};
	std::ostringstream oss;
	if (floating < 0 || floating > 127)
		info[CHAR] = "KO";
	char	character = static_cast<char>(floating);
	if (!std::isprint(character) && info[CHAR] == "OK")
		info[CHAR] = "N/A";
	if (floating < std::numeric_limits<int>::min() || floating > std::numeric_limits<int>::max() )
		info[INT] = "KO";
...
void	display_convert(double double_floating, int decimal){
	std::string info[4] = {"OK", "OK", "OK", "OK"};
	std::ostringstream oss;
	if (double_floating < 0 || double_floating > 127)
		info[CHAR] = "KO";
	char	character = static_cast<char>(double_floating);
	if (!std::isprint(character) && info[CHAR] == "OK")
		info[CHAR] = "N/A";
	if (double_floating < std::numeric_limits<int>::min() || double_floating > std::numeric_limits<int>::max() )
		info[INT] = "KO";
	int		integer = double_floating;
	// if (double_floating < -std::numeric_limits<float>::max() || double_floating > std::numeric_limits<float>::max() )
	// 	info[FLOAT] = "KO";
...
void	convertToAllType(std::string &input, void *ptr, int type){	
	...
	switch (type)
	{
		...
		case 2:
		{
			...
			display_convert(floating, count_decimal(input));
			break ;
		}
		case 3:
		{
			double_floating = static_cast<long double*>(ptr)[0];
			display_convert(double_floating, count_decimal(input));
			break ;
		}
	}
